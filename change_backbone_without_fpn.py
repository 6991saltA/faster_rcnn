...
def create_model(num_classes):
    ...
    # 要使用change_backbone要把torch升级到1.10或以上的版本

    # 如果要使用自己搭建的backbone网络，就将构建模型部分(torchvision.models.***改成实例化自己模型的类即可)
    # 就像train_res50_fpn.py中的resnet50_fpn_backbone类一样

    # 对于FPN, RPN, 以及faster rcnn部分的权重都是没有经过学习，所以直接拿来训练自己的数据集效果会比较差，
    # 建议先用自己构建的模型先在coco数据集上进行预训练，训练完之后在拿到自己的数据集进行迁移训练
    # 想在coco数据集上进行与训练的话，可以参考pytorch_object_detection/train_coco_dataset
